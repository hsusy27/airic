...
`label` 是左邊的類別，`value` 是撥號方式（會用等寬字放大顯示），`note` 是右邊的小灰標籤，不需要就寫 `note: ''`。

---

### 5.6.1 改電話分線面板

還是 `phone:` 區塊裡面，多了一個 `panel:`。這是取代原本手繪電話圖的按鍵面板，**不用畫圖**，改數字就好：

```js
panel: [
  { key: 1,  ext: '12561' },
  { key: 2,  ext: '12562' },
  { key: 13, ext: '12587', outside: true },   // 外線會多顯示一個「外線」小標籤
  { key: 16, empty: true },                    // 這顆鍵還沒設定，畫面顯示空鍵、不能點
],
```
...
|---|---|---|
| `key` | 按鍵編號 | 面板上印的數字，必填 |
| `ext` | 對應分機／線路號碼 | 點下按鍵時顯示出來的號碼 |
| `outside` | 是否為外線 | 寫 `true` 會多顯示「外線」標籤，一般分機不用寫這行 |
| `empty` | 是否為空鍵 | 寫 `true` 代表還沒設定，會顯示成灰色空鍵、不能點；有寫 `empty` 就不用再寫 `ext` |

**排列順序＝陣列順序**：面板會照你在 `panel:` 裡寫的先後順序，由左到右、由上到下排出來，跟 `key` 數字本身無關（`key` 只是顯示在按鍵上的文字）。想調整排列，直接搬動整筆 `{ ... }` 的位置即可。

> ⚠️ 目前這份對照表是依你提供的面板照片與分機表回推的初版，第 16～24 鍵照片上看不清楚標籤先留空。**上線前請對照實體電話面板確認每一顆鍵，修正錯誤或補上空缺的鍵。**

---

### 5.7 改頁首數字、窗口、更新日期

- `phone:` —— 桌上電話的撥號規則（學校／醫院／簡碼／外縣市）
